...
const Mutation = {
    createUser(parent,args,{db},info){
        ...
        const user = {
            id: uuidv4(),
            ...args.data //aşağıdaki posttaki args yerine spread operatörü kullanarak argstaki bütün propları kopyalayabilirsin
        }
        db.users.push(user)
        return user
    },
    ...
    createPost(parent,args,{db, pubsub},info){
        const userCheck = db.users.some((user)=>user.id ===args.data.author)
        if(!userCheck){
    ...
    createComment(parents,args,{db , pubsub },info){
        const userCheck = db.users.some((user)=> user.id ===args.data.author)
        const postCheck = db.posts.some((post)=> post.id === args.data.post && post.published)
        if(!userCheck){
            throw new Error('User does not exist')
        }
        else if(!postCheck){
